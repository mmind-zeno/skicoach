...
**Release (Webapp):** `0.10.2` — siehe `webapp/package.json`, Docker-Build-Arg `NEXT_PUBLIC_APP_VERSION` (Default in Repo-Root `docker-compose.yml`). Live: `https://skicoach.mmind.space`.
...
- Team-Chat (Socket.io)
- Admin-Panel (inkl. **Kommunikation** `/admin/kommunikation`: Terminerinnerungen Zeiten/Kanal/Webhook, Newsletter an Marketing-Opt-in-Gäste)
...
### Termin-Erinnerungen (E-Mail / SMS)

- **Service:** `webapp/src/services/booking-reminder.service.ts` — `runDueBookingReminders()`; Scheduler in `server.ts` alle **60s**, tatsächlicher Abstand aus **DB/UI** (`app_settings`) oder Env-Default.
- **Log:** `booking_reminder_log` — **max. ein Eintrag pro Buchung** (Migration **`0019`**).
- **Pro Gast:** `guests.booking_reminder_opt_in` (Migration **`0018`**). Kanal: **E-Mail**, **SMS (Webhook)** oder **beides** — Admin `/admin/kommunikation` oder Keys in `app_settings` (`reminder_medium`, `reminder_sms_webhook_url`, …).
- **Env-Fallback:** `REMINDER_*` wenn keine DB-Zeilen; UI „Zurücksetzen“ löscht `app_settings`-Einträge.
- **Newsletter:** Tabelle `newsletter_campaigns` (**0019**), Versand nur an Gäste mit `marketing_opt_in` und E-Mail; Resend + Layout `lib/mail/newsletter-layout.ts`.
...
*Next.js 14 · Drizzle · PostgreSQL 16 · TypeScript strict · react-big-calendar · Hetzner · skicoach.mmind.space · 49.13.139.206 · **v0.10.2***
