...
**Release-Version:** `webapp/package.json` (aktuell 0.5.0) — optional `NEXT_PUBLIC_APP_VERSION` als Docker-Build-Arg; `GET /api/public/health` liefert `version`; App-Container führt beim Start `npm run db:migrate` aus (`docker-entrypoint.sh`).
