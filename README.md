...
**Release-Version:** `webapp/package.json` (aktuell **0.10.3**) — optional `NEXT_PUBLIC_APP_VERSION` als Docker-Build-Arg (Defaults in `docker-compose.yml` / `webapp/Dockerfile`); `GET /api/public/health` liefert `version`; App-Container führt beim Start `npm run db:migrate` aus (`docker-entrypoint.sh`).
