...
export default function DatenschutzPage() {
  ...
  return (
    <div className="public-safe-x mx-auto max-w-2xl px-0 py-8 sm:py-10">
      <h1 className={h1}>{L.navPrivacy}</h1>
      ...
      <p className={`mt-4 ${muted}`}>{L.privacyMusterDisclaimer}</p>
      <div className={body}>
        {sections.map((s) => (
          <section key={s.title}>
      ...
      <p className={`mt-6 ${muted}`}>
        ...
        <a className={link} href={L.privacyHostingNoteLinkUrl} target="_blank" rel="noreferrer">
